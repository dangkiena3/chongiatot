...
if(is_post()){
	...
	if($_POST['createnew']){
		if(empty($_POST['name_info'])){
			...
			redirect( WEB_ROOT. "/manage/cate/index2.php?type=".$_POST['type_info']);
		}
		$insert	=	array('pid','type','name','sort_order','display','image_cate','desc','keyword');
		...
		if(!empty($newcate)) {
			...
			header("location: ".WEB_ROOT."/manage/cate/index2.php?type=".$_POST['type_info']);
