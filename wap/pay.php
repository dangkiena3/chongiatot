...
//payed order
if ( $order['state'] == 'pay' ) {  
	Session::Set('notice', 'Bạn đã thanh toán sản phẩm này rồi');
...
if ($login_user['money'] >= $order['origin']) { 
	...
	Session::Set('error', 'Số dư trong tài khoản không đủ để thanh toán, vui lòng nạp thêm tiền!');
	redirect('index.php');
}
...
if ( $_POST['service'] == 'credit' ) {
	...
	ZTeam::BuyOne($order);
	Session::Set('notice', 'conseguiu comprar');
