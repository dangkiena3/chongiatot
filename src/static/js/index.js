...
var validator = {
    ...
    isIdCard: function (number) {
        ...
        var area = ['', '', '', '', '', '', '', '', '', '', '', 'åŒ—äº¬', 'å¤©æ´¥', 'æ²³åŒ—', 'å±±è¥¿', 'å†…è’™å�¤', '', '', '', '', '', 'è¾½å®�', 'å�‰æž—', 'é»‘é¾™æ±Ÿ', '', '', '', '', '', '', '', 'ä¸Šæµ·', 'æ±Ÿè‹�', 'æµ™æ±Ÿ', 'å®‰å¾®', 'ç¦�å»º', 'æ±Ÿè¥¿', 'å±±ä¸œ', '', '', '', 'æ²³å�—', 'æ¹–åŒ—', 'æ¹–å�—', 'å¹¿ä¸œ', 'å¹¿è¥¿', 'æµ·å�—', '', '', '', 'é‡�åº†', 'å››å·�', 'è´µå·ž', 'äº‘å�—', 'è¥¿è—�', '', '', '', '', '', '', 'é™•è¥¿', 'ç”˜è‚ƒ', 'é�’æµ·', 'å®�å¤�', 'æ–°ç–†', '', '', '', '', '', 'å�°æ¹¾', '', '', '', '', '', '', '', '', '', 'é¦™æ¸¯', 'æ¾³é—¨', '', '', '', '', '', '', '', '', 'å›½å¤–'];
...
//------ menu dang ký----------------
window.x_init_hook_openid_register = function () {
    var ma = jQuery('#openid-register');
    var mm = jQuery('#openid-register-menu');
    ma.hover(function () {
        mm.show();
        ma.toggleClass('hover', true)
    }, function () {
...
window.x_init_hook_clock = function () {
    ...
    var e = function () {
        ...
        if (ls > 0) {
			...
			if(lh<10) {lh = '&nbsp;0'+lh}
			else{ if(lh<100) {lh = '&nbsp;'+lh;}}
			if(lm<10) {lm = '0'+lm};
			if(ls<10) {ls = '0'+ls};
            var html = '<li>' + lh + '</li><li>:</li><li>' + lm + '</li><li>:</li><li>' + ls + '</li>';
            jQuery('ul#counter').html(html)
        } else {
            jQuery("ul#counter").stopTime('counter');
            jQuery('ul#counter').html('');
            window.location.reload()
        }
    };
...
window.x_init_hook_team = function () {
    jQuery('#deal-buy-quantity-input').bind("keyup", function () {
        ...
        n = isNaN(n) ? 0 : n;
        var t = n * p;
        jQuery('#deal-buy-total').html(t.toFixed(0)+'đ');
...
X.team.dealbuy_totalprice = function () {
    ...
    p = isNaN(p) ? 0 : p;
    var t = n + p;
    jQuery('#deal-buy-total-t').html(t.toFixed(0)+'đ')
};
...
X.manage.orderrefund = function () {
    ...
    if (oid && rid && user_id) return !X.get(WEB_ROOT + '/ajax/manage.php?action=orderrefund&id=' + oid + '&rid=' + rid + '&user_id=' + user_id)
};
