...
elseif ( 'update' == $action) {
	...
	$update = strtotime("+10 day", strtotime(date('Y-m-d H:i',$team['end_time'])));
	Table::UpdateCache('team', $id, array('end_time' => $update,));
	Session::Set('notice', 'Đã gia hạn deal <b>'.$id.'</b> thêm <b>'.$day.'</b> ngày');
	json(null, 'refresh');
}
...
else if ( 'teamactive' == $action) {
	need_auth('team');
	$id = strval($_GET['id']);
	$p = strval($_GET['p']);
	$txt = $p ? 'đã ẩn' : 'đã hiển thị';
	$flag = Table::UpdateCache('team', $id, array(
				'active' => $p,
				));
	if($flag){
		Session::Set('notice', "Sản phẩm {$id} {$txt}");
		json(null, 'refresh');
	}
}
...
// remove bank
	...
	$id = strval($_GET['id']);
	$flag = Table::UpdateCache('vncountry', $id, array(
				'vat' => 0,
				));
	if($flag){
...
// set đã duyệt bản tin này
	...
	$order = Table::Fetch('order', $teamorder['order_id']);
	$user = Table::Fetch('user', $order['user_id']);
	$team_orders = DB::LimitQuery('team_order', array('condition'=>array('id'=>$id)));
	$team = Table::Fetch('team', $order['team_id']);
	$html = render('manage_ajax_dialog_orderdetail');
	json($html, 'dialog');
}
else if ( 'editOrder' == $action) {
	$teamorder = Table::Fetch('team_order', $id);
	$order = Table::Fetch('order', $teamorder['order_id']);
	$user = Table::Fetch('user', $order['user_id']);
	$team_orders = DB::LimitQuery('team_order', array('condition'=>array('id'=>$id)));
