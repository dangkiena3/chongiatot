...
if ($vote_today) {
	Session::Set('notice', 'Have you participated in the survey today!');
...
if (!count($vote_list)) {
	Session::Set('error', 'Content may not be empty.');
	redirect( WEB_ROOT . '/vote/index.php');	
}
...
if (!$feedback_id) {
	Session::Set('error', 'not adding, database operation failed.');
	redirect( WEB_ROOT . '/vote/index.php');	
}
...
if (is_post()) {
	Session::Set('notice', 'Thanks for your vote!');
}
redirect( WEB_ROOT . '/vote/index.php');	
