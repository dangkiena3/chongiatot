...
class ZFlow {
	...
	static public function Explain($record=array()) {
		...
		if ('buy' == $action) {
			...
			if ($team) {
				return  "item bought - <a href=\"/team.php?id={$team['id']}\">{$team['title']}</a>";
			} else {
				return "item bought - this item has been deleted";
			}
		}
		else if ( 'invite' == $action) {
			$user = Table::Fetch('user', $record['user_id']);
			return "invitation rebate - friend{$user['username']}registered and bought";
		}
		else if ( 'store' == $action) {
			return 'charge with cash';
		}
		else if ( 'withdraw' == $action) {
			return 'cash withdraw';
		}
		else if ( 'coupon' == $action) {
			return "consumption rebate - conpon consumption rebate";
		}
		else if ( 'refund' == $action) {
			$team = Table::Fetch('team', $record['detail_id']);
			if ($team) {
				return  "refund policy - <a href=\"/team.php?id={$team['id']}\">{$team['title']}</a>";
			} else {
				return "item refund - the item has been deleted";
			}
		}
