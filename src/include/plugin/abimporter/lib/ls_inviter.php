...
//@api
function ls_get_error_message($err)
{
	switch ($err) {
...
//@api
function ls_translate_error($errcode)
{
	switch ($errcode) {
...
//@api
function ls_sort_compare_contacts(&$c1,&$c2)
{
	return strcasecmp($c1['name'],$c2['name']);
}

//@api
function ls_translate_contacts($contacts)
{
	$cl = array();
	foreach ($contacts as $c)
	{
		if (ls_instanceof($c,'Contact'))
		{
			$c2 = array();
			$c2['type'] = 'email';
			$c2['id'] = $c->email;
		...
		else if (ls_instanceof($c,'SocialContact'))
		{
			$c2 = array();
			$c2['type'] = 'social';
			$c2['id'] = $c->uid;
	...
	usort($cl, "ls_sort_compare_contacts");
	return $cl;
}
...
class OzInviter
{
	...
	function _updateConfig() {
		if ($this->config) foreach ($this->config as $k=>$v) ls_set_config($k,$v);
	}
	...
	function login ($loginemail, $password, $service='', $listtype=NULL)
	{
		...
		if (method_exists($this->obj,'sendMessages'))
		{
			...
			if (ls_instanceof($res,'CaptchaChallenge'))
			{
				$this->captcha = $res;
				return OZE_CAPTCHA;
			}
	...
	function fetch_contacts()
	{
		...
		return ls_translate_contacts($this->contacts);
	}
	...
	{
		return ls_is_service_supported($svcid);
	}
	...
	function send_messages ($recipients, $subject, $message, $format='text')
	{
		...
		if (method_exists($this->obj,'sendMessages'))
		{
			...
			if (is_int($res)) return ls_translate_error($res);
		...
		else
		{
			...
			/*
			//It's email based
			if (function_exists('ls_send_messages'))
			{
				return ls_send_messages($recipients,$subject,$message,$format);
			}
			else
			{
				return OZE_UNSUPPORTED_OPERATION;
	...
	{
		$this->_updateConfig();
		$file = ls_convert_charset($file, $charset, 'UTF-8');
		...
		if (is_array($res)) return ls_translate_contacts($res);
