...
function ls_vcard_unescape ($str) {
...
function ls_vcard_to_string($ba,$encoding,$charset) {
	if ($encoding!=null) {
...
function ls_vcard_to_binary ($ba, $encoding) {
	if ($encoding!=null) {
		$encoding = strtoupper($encoding);
...
function ls_vcard_split ($ba,$delim=',') {
	$a = array();
	$n = strlen($ba);
	$i1 = 0;
	for ($i = 0; $i < $n; $i++) {
...
class OzVCardField {
	...
	function getBinaryValue () {
		...
		return ls_vcard_to_binary($this->rawValue,$encoding);
	}

	function getStringValue () {
		$encoding = $this->getFirstParamValue('ENCODING');
		$charset = $this->getFirstParamValue('CHARSET');
		return ls_vcard_to_string($this->rawValue,$encoding,$charset);
	}
	...
	function getStringValues ($delim) {
		...
		$charset = $this->getFirstParamValue('CHARSET');
		$al = array();
		$a = ls_vcard_split($this->rawValue,$delim);
		foreach ($a as $v)
			$al[] = ls_vcard_to_string($v,$encoding,$charset);
		return $al;
	}
}
