...
function ls_json_decode ($json, $assoc=FALSE) {

	//PHP's current json handling is unreliable.
	//Quercus's JSON is assumed to be reliable.
	$reliable_json = ls_get_config('reliable_json',NULL);
	if ($reliable_json===NULL) $reliable_json = ls_is_quercus();
...
function ls_json_decode2 ($json, $assoc=FALSE) {
	$p = new OzJsonParser($json,$assoc);
	$readSomething=FALSE;
	return $p->readValue($readSomething);
	//$p = new OzJsonParser($json,$assoc);
...
function ls_php_escape_string(&$s) {
	$n = strlen($s);
	$s2='';
	for ($i=0;$i<$n;$i++) {
		$c = $s[$i];
...
function ls_json_escape_string(&$s) {
    $n = strlen($s);
    $s2='';
    for ($i=0;$i<$n;$i++) {
        $c = $s[$i];
...
function _ls_json_encode (&$obj) {
    ...
    elseif (is_array($obj)) {
        ...
        if (count($obj)===0) return '[]';
            ...
            foreach ($obj as $k=>$v) {
                if ($c>0) $s .= ',';
                $s .= ls_json_encode($k);   //$k assumes to be valid json names!
                $s .= ':';
                $s .= ls_json_encode($v);
                $c++;
            }
...
function ls_json_encode ($obj) {
	if (function_exists('json_encode')) return json_encode($obj);
	else return _ls_json_encode($obj);
}
