...
define('OrkutImporter_CONTACTEMAIL_REGEX',"/<div[^>]*?class=\"nor\\s*\"[^>]*?>([^<]*)/ims");
define('OrkutImporter_CONTACTNAME_REGEX',"/<h3[^>]*class=\"smller[^>]*>\\s*(?:<a\\s+[^>]*>)?([^<]*)</ims");
define('OrkutImporter_NEXTPAGE2_REGEX',"/<a\\s+href=\"(\/Friends\\.aspx\\?show=all&pno=\\d+)\"[^>]*>[^<]+?&gt;<\/a>/ims");
...
class OrkutImporter extends WebRequestor {
	...
	function login ($loginemail, $password) {
		...
		$form = ls_extract_form_by_id($html,'gaia_loginform');
		...
		$location = ls_get_refresh_url($html);
		if ($location!=null) 
			$html = $this->httpGet($location);
		return _ABI_SUCCESS;		
	}
	...
	function fetchContacts ($loginemail=NULL, $password=NULL) {
	 	...
	 	$maxPages = 50;
    	$html = $this->httpGet('/Friends.aspx?show=all&pno=1');
		$al = array();
