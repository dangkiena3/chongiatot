...
class GMailImporter2 extends WebRequestor {
	...
	function authSub($loginEmail, $authSubToken) {
		...
		ls_set_domain(ls_get_email_domain($loginEmail));
	...
	function login ($loginEmail, $password, $captchaToken=null, $captchaAnswer=null) {
		...
		$form->addField("source", "Lunarsys-ABI-1");
	...
	function makeGmailAuthHeaders ($url) {
		...
		if (ls_get_config('gmail.secure',FALSE)) {
			$privKeyFilePath = ls_get_config('gmail.ssl_private_key',NULL);
			if ($privKeyFilePath===NULL) $privKeyFilePath = ls_get_config('ssl.private_key');	//Backward compatibility
			$timestamp = time();  
...
//		$url = "http://www.google.com/m8/feeds/contacts/$this->loginEmail/full?max-results=10000";
		...
		$limitToMyContacts = ls_get_config('gmail.limit_mycontacts',NULL);
		if ($limitToMyContacts===NULL) {
			$filter = ls_get_config('gmail.filter','all');
			if ($filter=='mycontacts') $limitToMyContacts=true;
		}
		//$limitToMyContacts = ls_get_config('gmail.limit_mycontacts',false);
		...
		if (preg_match(GMailImporter2_ID_REGEX,$entryHtml,$matches)) {
			...
			//ls_set_domain(ls_get_email_domain($user_id));
		}
		*/
