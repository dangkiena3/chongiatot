...
class Contact {
	...
	function Contact ($name=null ,$email=null) {
		if (empty($name) && ls_get_config('email_as_name',TRUE))
			$name=$email;
		$this->name = $name;
		$this->email = $email;
	}
...
function abi_housekeep_captcha($path) {
	...
	if (ls_get_config('housekeep_captcha',FALSE)) {
		$oldest = time()-(30*60);
...
function abi_captcha_filepath () {
	$path = ls_get_config('captcha_file_path','./captcha');
...
function abi_captcha_uripath () {
	return ls_get_config('captcha_uri_path','./captcha');
}


function ls_is_debug() {return ls_get_config('debug',FALSE);}
...
class WebRequestor {
	...
	function setOwnerEmail($email) {
		...
		if ($email!==NULL) { 
			$parts = ls_get_email_parts ($email);
			if (count($parts)>=2) ls_set_domain($parts[1]);
			else ls_set_domain(NULL);
		}
		else {
			ls_set_domain(NULL);
		}
	...
	function WebRequestor () {
	  ...
	  $this->useHttp1_1 = ls_get_config('http1_1',FALSE);
	}

	function enableHttp1_1Features ($enabled) {
	  $this->useHttp1_1 = ls_get_config('http1_1',FALSE) ? $enabled : false;
	}
...
*/
	...
	function httpRequest ($url, $ispost=false, $postData=null, $defaultCharset='iso-8859-1',$extraHeaders=null,$overrideCharset=null) {
		...
		while (TRUE) {
			...
			if (!ls_is_quercus()) curl_setopt($this->ch, CURLOPT_BINARYTRANSFER, 1);
...
//			curl_setopt($this->ch, CURLOPT_VERBOSE, 1);
			...
			if (ls_get_config('curl_proxy','')!='') {curl_setopt($this->ch, CURLOPT_PROXY, ls_get_config('curl_proxy'));$using_proxy=TRUE;}
			if (ls_get_config('curl_proxyport','')!='') {curl_setopt($this->ch, CURLOPT_PROXYPORT, ls_get_config('curl_proxyport'));$using_proxy=TRUE;}
			if (ls_get_config('curl_proxytype','')!='') {curl_setopt($this->ch, CURLOPT_PROXYTYPE, ls_get_config('curl_proxytype'));$using_proxy=TRUE;}
			...
			if (ls_get_config('gzip',TRUE) && $this->supportGzip && defined('CURLOPT_ENCODING')) curl_setopt($this->ch, CURLOPT_ENCODING, "gzip");
			$ts = $f();
...
//		return _ABI_FAILED;
	...
	function makeAbsolute($absolute, $relative) {
		return ls_make_absolute_url ($absolute, $relative);
	}


	//Return email in array of id, domain
	function getEmailParts ($email) {
		return ls_get_email_parts($email);
	}
...
//	}
	...
	function reduceWhitespace ($str) {
		return ls_reduce_whitespace ($str);
	}
...
//RFC822 date parser
//-----------------------------------------------------------------------------------
global $ls_w3c_TimeZones;
global $ls_w3c_Months;

$ls_w3c_Months = array('jan'=>1,'feb'=>2,'mar'=>3,'apr'=>4,'may'=>5,'jun'=>6,'jul'=>7,'aug'=>8,'sep'=>9,'oct'=>10,'nov'=>11,'dec'=>12,'january'=>1,'february'=>2,'march'=>3,'april'=>4,'may'=>5,'june'=>6,'july'=>7,'august'=>8,'september'=>9,'october'=>10,'november'=>11,'december'=>12);

$ls_w3c_TimeZones = array(
...
function w3c_getTimeZoneOffset($tz) {
	...
	global $ls_w3c_TimeZones;
	$result = null;
	foreach ($ls_w3c_TimeZones as $sa)	{
...
function ls_jsdecode_quoted_string ($js) {
	$sb = '';
	$n = strlen($js);
	if ($n<2) return $js;
	$escapeChar = false;
...
/*
function ls_jsdecode_quoted_string($strwithquotes) {
	$js = '[v:'.$strwithquotes.']';
	$js = '[\'hel\\"lo\']';
	echo $js;
...
function ls_reduce_whitespace ($str) {
	$sb = '';
...
function ls_make_absolute_url($absolute, $relative) {

	//If relative is an absolute path, return it
	if (empty($relative)) {
		$relative=$absolute;
	}
	$p = ls_parse_url($relative);

	if(isset($p["scheme"])) return $relative;
	$path = isset($p["path"]) ? $p["path"] : "";
	$path = dirname($path);

	extract(ls_parse_url($absolute));
	if($relative{0} == '/') {
...
function ls_extract_form_by_name($html, $name) {
	//$name = strtolower($name);
	$res = ls_extract_forms($html);
	foreach ($res as $fo) {
...
function ls_extract_form_by_id($html, $formId) {
...
function ls_extract_forms ($html,$onlyHidden=true) {
...
function ls_instanceof(&$obj, $type) {
	if (ls_is_php5()) {
...
function ls_get_refresh_url($html) {
...
function ls_extract_params($url) {
	$map = array();
	$i = strpos($url,'?');
...
function ls_extract_param($url,$param,$defaultVal=NULL) {
	$map = ls_extract_params($url);
...
//@api
function ls_get_supported_services($type=NULL)
{
	global $_OZ_SERVICES;
...
//@api
function ls_is_service_supported($id)
{
	global $_OZ_SERVICES;
...
//@api
function ls_get_service_info($svcid) {
	global $_OZ_SERVICES;
...
//@api
function ls_get_supported_domains()
{
	global $_DOMAIN_IMPORTERS;
...
function abi_email_as_name() {
	return ls_get_config('email_as_name',TRUE);
}
...
//@api
function ls_array_to_php(&$arr,$indent=0) {
	$s= 'array(';
	if ($indent>=0) $s.="\r\n";
...
//Return email in array of id, domain
function ls_get_email_parts ($email) {
...
function ls_get_email_domain ($email) {
	$parts = ls_get_email_parts($email);
	return $parts[1];
}

function ls_convert_charset ($str, $from, $to) {
